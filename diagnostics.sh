...
if [ $pct -ge "90" ] && [ -z "$researchmailsent" ]; then
	mail -s "Research Homedirs is over 90% full!!! Do something, Q!" qread@sesync.org < /dev/null
...
if [ ${nbadnodes[2]} -ge "6" ] && [ -z "$slurmmailsent" ]; then
	mail -s "Over 1/4 of the slurm nodes are in a bad state!!! Do something, Q!" qread@sesync.org < /dev/null
